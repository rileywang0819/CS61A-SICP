...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    total_score = 0
    outcome_one_occur = False
    while num_rolls > 0:
        current_score = dice()
        if current_score == 1:
            outcome_one_occur = True
        else:
            total_score += current_score
        num_rolls -= 1
    return 1 if outcome_one_occur else total_score
    # END PROBLEM 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    return roll_dice(num_rolls, dice) if num_rolls else tail_points(opponent_score) 
    # END PROBLEM 3
...
# BEGIN PROBLEM 4
def square_root(n):
    """Return the square root if there is an interger i satisfying i * i = n, 
    otherwise return -1.
...
def perfect_square(score):
    """Return True if the given score is a perfect square, otherwise
    return False
    ...
    False
    """
    pseudo_root = square_root(score)
    return False if pseudo_root == -1 else True

def next_perfect_square(score):
    root = square_root(score)
    return (root + 1) ** 2
# END PROBLEM 4
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def roll_dice(score, opponent_score):
        return n
    return roll_dice
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    prev_num_dice = -1      # number of dice given by strategy function
    while score <= goal and opponent_score <= goal:
        if prev_num_dice < 0:
            prev_num_dice = strategy(score, opponent_score)
            opponent_score += 1
            continue
        curr_num_dice = strategy(score, opponent_score)
        if prev_num_dice != curr_num_dice:
            return False
        elif score <= goal and opponent_score < goal:
            opponent_score += 1
        else:
            score += 1
            opponent_score = 0
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def roll_dice_samples(*args):
        nonlocal total_samples
        times, total = total_samples, 0
        while times > 0:
            total += original_function(*args)
            times -= 1
        return total / total_samples
    return roll_dice_samples
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    highest_average, highest_average_num_dice = 0, 0
    num_dice = 1
    while num_dice <= 10:
        curr_average = make_averaged(roll_dice, total_samples)(num_dice, dice)
        if curr_average > highest_average:
            # update the highest average and the corresponding num of dices
            highest_average = curr_average
            highest_average_num_dice = num_dice
        num_dice += 1
    return highest_average_num_dice
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    gain = square_update(0, score, opponent_score) - score
    return 0 if gain >= threshold else num_rolls
    # END PROBLEM 11
